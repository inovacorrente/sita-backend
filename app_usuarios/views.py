...
class CustomTokenObtainPairView(TokenObtainPairView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if response.status_code == 200:
            return Response(SuccessResponse.login_success(response.data), status=status.HTTP_200_OK)
        return response
...
@extend_schema_view(
    get=extend_schema(
        ...
        description="Lista todos os usuários cadastrados. Requer autenticação e permissão adequada.",
        parameters=[
            OpenApiParameter("search", str, description="Busca por matrícula, nome, email ou CPF"),
            OpenApiParameter("is_active", bool, description="Filtra por status ativo"),
            OpenApiParameter("is_staff", bool, description="Filtra por administradores staff"),
            OpenApiParameter("is_superuser", bool, description="Filtra por superusuários"),
        ],
        responses={
            200: OpenApiResponse(
                response=UsuarioCustomViewSerializer(many=True),
                description="Lista de usuários retornada com sucesso"
            ),
            403: OpenApiResponse(description="Sem permissão para listar usuários")
        }
    )
)
...
@extend_schema_view(
    post=extend_schema(
        ...
        description="Cria um novo usuário no sistema. Apenas administradores podem criar outros administradores.",
        request=UsuarioCustomCreateSerializer,
        responses={
            201: UsuarioCustomViewSerializer,
            400: OpenApiResponse(description="Erro de validação"),
            403: OpenApiResponse(description="Sem permissão para criar este usuário")
        }
    )
)
...
# ============================================================================
# DETALHE E ATUALIZAÇÃO DE USUÁRIO
# ============================================================================
...
@extend_schema_view(
    get=extend_schema(
        ...
        description=(
            "Retorna os detalhes de um usuário específico pela matrícula.\n\n"
        ...
        responses={
            ...
            403: OpenApiResponse(description="Sem permissão para visualizar este usuário"),
...
class UsuarioMeView(APIView):
    ...
    def get(self, request):
        ...
        return Response(
            SuccessResponse.retrieved(serializer.data, "Dados recuperados com sucesso."),
    ...
    def put(self, request):
        try:
            ...
            if serializer.is_valid():
                ...
                return Response(
                    SuccessResponse.updated(serializer.data, "Dados atualizados com sucesso."),
                    status=status.HTTP_200_OK
                )
            logger.warning(f"Erro de validação para usuário {request.user.matricula}: {serializer.errors}")
            return Response(format_error_response(serializer.errors, 400), status=status.HTTP_400_BAD_REQUEST)
        except Exception as e:
            logger.error(f"Erro inesperado ao atualizar dados do usuário {request.user.matricula}: {str(e)}")
            return Response(format_error_response("Ocorreu um erro inesperado.", 500), status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
@extend_schema(
    ...
    description=(
        ...
        "- **Admins**: podem alterar qualquer usuário.\n"
        "- **Usuários comuns**: apenas a si mesmos."
    ),
    parameters=[
        OpenApiParameter("matricula", str, location=OpenApiParameter.PATH, description="Matrícula do usuário")
    ],
    responses={
...
class UsuarioAtivarDesativarView(APIView):
    ...
    def patch(self, request, matricula, *args, **kwargs):
        if not matricula:
            return Response(ValidationErrorResponse.required_field('matricula'), status=status.HTTP_400_BAD_REQUEST)

        if request.user.is_staff or request.user.is_superuser:
            try:
                user = UsuarioCustom.objects.get(matricula=matricula)
            except UsuarioCustom.DoesNotExist:
                return Response(ValidationErrorResponse.user_not_found(), status=status.HTTP_404_NOT_FOUND)
        else:
            user = request.user
            if matricula != user.matricula:
                return Response(ValidationErrorResponse.permission_denied(), status=status.HTTP_403_FORBIDDEN)

        user.is_active = not user.is_active
        user.save()
        logger.info(f"Usuário {user.matricula} foi {'ativado' if user.is_active else 'desativado'} por {request.user.matricula}")

        return Response(
            SuccessResponse.updated({'matricula': user.matricula, 'is_active': user.is_active},
